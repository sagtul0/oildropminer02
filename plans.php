...
$stmt->execute(['chat_id' => $chat_id]);
$user_blocked = $stmt->fetch(PDO::FETCH_ASSOC);
...
$stmt = $conn->prepare("SELECT balance, boost_multiplier, auto_clicker, auto_clicker_expiration FROM users WHERE chat_id = :chat_id");
$stmt->execute(['chat_id' => $chat_id]);
$user = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$user) {
    error_log("User not found for Chat_ID: $chat_id in plans.php");
    ...
    if (!isset($plans[$plan])) {
        ...
        if ($balance < $plan_price) {
            ...
            if (isset($plan_info['multiplier'])) {
                ...
                $stmt = $conn->prepare("UPDATE users SET balance = :balance, boost_multiplier = :boost_multiplier WHERE chat_id = :chat_id");
                $stmt->execute(['balance' => $new_balance, 'boost_multiplier' => $new_multiplier, 'chat_id' => $chat_id]);
                ...
                $stmt = $conn->prepare("UPDATE users SET balance = :balance, auto_clicker = true, auto_clicker_expiration = :expiration WHERE chat_id = :chat_id");
                $stmt->execute(['balance' => $new_balance, 'expiration' => $expiration_date, 'chat_id' => $chat_id]);
            }
